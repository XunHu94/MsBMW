  !***********************************************************************************
    SUBROUTINE InferCpdfRMSE(ix,iy,iz,imult)
    !c----------------------------------------------------------------------
    !c
    !c----------------------------------------------------------------------
    use varmod
    ...
    real, DIMENSION (MAXX,MAXY) :: ESnySim1,ESyntheticSeismic1,VarSnySim1,VarSyntheticSeismic1,ESnySim2, ESyntheticSeismic2, CovSny2, VarSnySim2, VarSyntheticSeismic2
...
45      continue
    ...
    IF(WCY.GT.1)THEN
        ...
        ESyntheticSeismic1(1:nx,1:ny) = 0.0
        VarSnySim1(1:nx,1:ny) = 0.0
        VarSyntheticSeismic1(1:nx,1:ny) = 0.0
        do ixTem=nctx1,nctx2,ncoarset
            iiix=ix+ixTem
            do iyTem=ncty1,ncty2,ncoarset
                iiiy=iy+iyTem
                
                ! obeject function 1:RMSE 
                call convolution(nz,iiix,iiiy,1,DenSim(iiix,iiiy,1:nz),VpSim(iiix,iiiy,1:nz),snytmp1(1:nz))
                do iiiz = nz, 1,-1
                    if(SNVar==0.0) then
                        lktmp1=lktmp1+abs(snytmp1(iiiz)-ActualSeismic(iiix,iiiy,iiiz))
                    else
                        lktmp1=lktmp1+(snytmp1(iiiz)-ActualSeismic(iiix,iiiy,iiiz))**2/(2*SNVar*SNVar)
                    end if
                end do
    ...
    ELSE
        lktmp1=10000000000000000000.0
    END IF
                                                   
    lktmp2=0.0
    ESnySim2(1:nx,1:ny) = 0.0
    ESyntheticSeismic2(1:nx,1:ny) = 0.0
    CovSny2(1:nx,1:ny) = 0.0
    VarSnySim2(1:nx,1:ny) = 0.0
    VarSyntheticSeismic2(1:nx,1:ny) = 0.0
    Rsny(1:nx,1:ny) = 0.0
    ...
    do ixTem=nctx1,nctx2,ncoarset
        ...
        do iyTem=ncty1,ncty2,ncoarset
            ...
            do iiiz = nz, 1,-1
                if(SNVar==0.0) then
                    lktmp2=lktmp2+abs(snytmp2(iiiz)-ActualSeismic(iiix,iiiy,iiiz))
                else
                    lktmp2=lktmp2+(snytmp2(iiiz)-ActualSeismic(iiix,iiiy,iiiz))**2/(2*SNVar*SNVar)
                end if
            end do
    ...
    call RANDOM_NUMBER(pptmp)
    Allcon=Allcon+1.0
    if(pptmp<exp((lktmp1-lktmp2)/Temperature)) then
        altcon=altcon+1.0
        BETTER=.TRUE.
        lktmp1=lktmp2
        do ixTem=nctx1,nctx2,ncoarset
