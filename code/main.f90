    ...
    open(timpar,file="Results\time.txt",status="UNKNOWN")
    OPEN(RMSEpar,file='Results\SeismicRMSE',status='UNKNOWN')
    OPEN(wellpar,file='Results\wells1',status='UNKNOWN')
    OPEN(recordpar,file='Results\recordpar1',status='UNKNOWN')
    ...
    open(dzsnycc,file="Results\zsnycc.txt",status='UNKNOWN')
    open(multigrid,file="Results\multigrid.txt",status='UNKNOWN')
    !
    ...
    DO WCY = 1,250
        ...
        FaciesOutfl(WCY)="Results\FaciesOut"// trim(adjustl(cTemp))
        SeismicOutfl(WCY)="Results\SeismicOut"// trim(adjustl(cTemp))
        syoutfl(WCY)="Results\rsnyOut"// trim(adjustl(cTemp))
        DenOutfl(WCY)="Results\DenOut"// trim(adjustl(cTemp))
        VpOutfl(WCY)="Results\VpOut"// trim(adjustl(cTemp))
        AIoutfl(WCY)="Results\AIOut"// trim(adjustl(cTemp))
        Seisrofl(WCY)="Results\Seisro"// trim(adjustl(cTemp))
